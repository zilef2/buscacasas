...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.Casaa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.Casaa[names['order']]
        });

        // form.codigo = props.Casaa?.codigo
    }
})

const update = () => {
    form.put(route('Casa.update', props.Casaa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.Casaa.Casa_id]"></v-select>
