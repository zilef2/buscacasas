...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.Horarioa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.Horarioa[names['order']]
        });

        // form.codigo = props.Horarioa?.codigo
    }
})

const update = () => {
    form.put(route('Horario.update', props.Horarioa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.Horarioa.Horario_id]"></v-select>
