...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.objetosa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.objetosa[names['order']]
        });

        // form.codigo = props.objetosa?.codigo
    }
})

const update = () => {
    form.put(route('objetos.update', props.objetosa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.objetosa.objetos_id]"></v-select>
