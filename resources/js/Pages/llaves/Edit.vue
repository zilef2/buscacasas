...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.llavesa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.llavesa[names['order']]
        });

        // form.codigo = props.llavesa?.codigo
    }
})

const update = () => {
    form.put(route('llaves.update', props.llavesa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.llavesa.llaves_id]"></v-select>
