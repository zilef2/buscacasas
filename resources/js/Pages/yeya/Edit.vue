...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.yeyaa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.yeyaa[names['order']]
        });

        // form.codigo = props.yeyaa?.codigo
    }
})

const update = () => {
    form.put(route('yeya.update', props.yeyaa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.yeyaa.yeya_id]"></v-select>
