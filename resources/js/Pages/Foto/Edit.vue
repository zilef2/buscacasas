...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.Fotoa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.Fotoa[names['order']]
        });

        // form.codigo = props.Fotoa?.codigo
    }
})

const update = () => {
    form.put(route('Foto.update', props.Fotoa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.Fotoa.Foto_id]"></v-select>
