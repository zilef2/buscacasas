...
import Create from '@/Pages/Apartamento/Create.vue';
import Edit from '@/Pages/Apartamento/Edit.vue';
import Delete from '@/Pages/Apartamento/Delete.vue';
...
watch(() => _.cloneDeep(data.params), debounce(() => {
    ...
    router.get(route("Apartamento.index"), params, {
        replace: true,
        preserveState: true,
        preserveScroll: true,
    })
...
const selectAll = (event) => {
    if (event.target.checked === false) {
        ...
        props.fromController?.data.forEach((Apartamento) => {
            data.selectedId.push(Apartamento.id)
        })
    }
}
const select = () => {
            ...
            <div class="px-4 sm:px-0">
                <div class="rounded-lg overflow-hidden w-fit">
                    <PrimaryButton class="rounded-none" @click="data.createOpen = true"
                        v-if="can(['create Apartamento'])">
                        {{ lang().button.new }}
                    </PrimaryButton>

                    <Create v-if="can(['create Apartamento'])" :numberPermissions="props.numberPermissions"
                        :titulos="titulos" :show="data.createOpen" @close="data.createOpen = false" :title="props.title"
                        :losSelect=props.losSelect />

                    <Edit v-if="can(['update Apartamento'])" :titulos="titulos"
                        :numberPermissions="props.numberPermissions" :show="data.editOpen" @close="data.editOpen = false"
                        :Apartamentoa="data.Apartamentoo" :title="props.title" :losSelect=props.losSelect />

                    <Delete v-if="can(['delete Apartamento'])" :numberPermissions="props.numberPermissions"
                        :show="data.deleteOpen" @close="data.deleteOpen = false" :Apartamentoa="data.Apartamentoo"
                        :title="props.title" />
                </div>
            </div>
                ...
                <div class="flex justify-between p-2">
                    <div class="flex space-x-2">
                        ...
                        <!-- <DangerButton @click="data.deleteBulkOpen = true"
                            v-show="data.selectedId.length != 0 && can(['delete Apartamento'])" class="px-3 py-1.5"
                        ...
                        <tbody>
                            <tr v-for="(claseFromController, indexu) in props.fromController.data" :key="indexu"
                                ...
                                <td v-if="numberPermissions > 1" class="whitespace-nowrap py-4 w-12 px-2 sm:py-3">
                                    <div class="flex justify-center items-center">
                                        <div class="rounded-md overflow-hidden">
                                            <InfoButton v-show="can(['update Apartamento'])" type="button"
                                                @click="(data.editOpen = true), (data.Apartamentoo = claseFromController)"
                                                class="px-2 py-1.5 rounded-none" v-tooltip="lang().tooltip.edit">
                                                <PencilIcon class="w-4 h-4" />
                                            </InfoButton>
                                            <DangerButton v-show="can(['delete Apartamento'])" type="button"
                                                @click="(data.deleteOpen = true), (data.Apartamentoo = claseFromController)"
                                ...
                                <td v-for="titulo in titulos" class="whitespace-nowrap py-4 px-2 sm:py-3">
                                    ...
                                    <span v-if="titulo['type'] === 'foreign'"> {{ claseApartamentoa[titulo['order']][titulo['nameid']] }} </span>
                                </td>
