...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.Apartamentoa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.Apartamentoa[names['order']]
        });

        // form.codigo = props.Apartamentoa?.codigo
    }
})

const update = () => {
    form.put(route('Apartamento.update', props.Apartamentoa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.Apartamentoa.Apartamento_id]"></v-select>
