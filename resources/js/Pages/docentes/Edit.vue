...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.docentesa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.docentesa[names['order']]
        });

        // form.codigo = props.docentesa?.codigo
    }
})

const update = () => {
    form.put(route('docentes.update', props.docentesa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.docentesa.docentes_id]"></v-select>
